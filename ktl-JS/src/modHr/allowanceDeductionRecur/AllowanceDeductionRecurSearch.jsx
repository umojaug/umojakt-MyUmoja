...
const AllowanceDeductionRecurSearch = ({ query }) => {
  const {
    data: list,
    error,
    isLoading,
    isError,
    refetch,
  } = useGetData("empallded", `/empAllDedRecur/search/${query}`);
          ...
          >
            ...
            <div>
              <div className="flex justify-end space-x-2">
                <DeleteButton
                  action={refetch}
                  path={`/empAllDedRecur/delete/${item.recurId}`}
                />
              </div>
            </div>
          </div>
...
export default AllowanceDeductionRecurSearch;
