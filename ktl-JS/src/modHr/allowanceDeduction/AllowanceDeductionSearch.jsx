...
const AllowanceDeductionSearch = ({ query }) => {
  const {
    data: list,
    error,
    isLoading,
    isError,
    refetch,
  } = useGetData("empallded", `/empallded/search/${query}`);
          ...
          >
            ...
            <div>
              <div className="flex justify-end space-x-2">
                <DeleteButton
                  action={refetch}
                  path={`/empallded/delete/${item.empAllDedId}`}
                />
              </div>
            </div>
          </div>
...
export default AllowanceDeductionSearch;
