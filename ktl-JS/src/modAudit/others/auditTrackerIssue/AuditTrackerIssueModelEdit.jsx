...
import AuditTrackerIssueModelFrom from "./AuditTrackerIssueModelFrom";

const AuditTrackerIssueModelEdit = ({ id }) => {
  ...
  const {
    ...
    isError,
    // refetch,
  } = useGetData("userlist", `/AuditTrackerIssue/details/${id}`);
          ...
          <div className="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4">
              <DialogPanel className="w-1/2 max-w-sm md:max-w-screen-md space-y-4 border bg-white rounded-lg">
                <AuditTrackerIssueModelFrom
                  closeModal={closeModal}
                  defaultValues={{
                    auditTrackerIssueId: list.data.auditTrackerIssueId,
                    issueStatus: list.data.issueStatus,
                    followUpDate:
                      list.data.followUpDate === "1980-12-31T00:00:00"
                        ? new Date()
                        : list.data.followUpDate,
                    implementationDate:
                      list.data.implementationDate === "1980-12-31T00:00:00"
                        ? new Date()
                        : list.data.implementationDate,
                    comments: list.data.comments,
                  }}
                  titleText="Edit Tracker Issue"
...
export default AuditTrackerIssueModelEdit;
