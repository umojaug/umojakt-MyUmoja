...
const EvaluationList = () => {
  const {
    ...
    isLoading,
    isError,
  } = useGetData("evaluation", "/evaluation/listbymanager");
  ...
  return (
    <div className="card w-full max-w-screen-xl">
      <TopHeader title="Application Received For Manager Review" />
          ...
          list.data.map((item) => (
            ...
            >
              ...
              <div className="flex justify-end space-x-2">
                {/* <TaskButton
                  path={`/evaluation/app/details/${item.evaluationId}`}
                /> */}
                {item.evaluationTypeName === "Annual Performance Review" && (
                  <TaskButton
                    path={`/evaluation/first/review/details/${item.evaluationId}`}
                    btnColor="btn-gray"
                  />
                )}
                {item.evaluationTypeName ===
                  ...
                  <TaskButton
                    path={`/evaluationThree/first/review/details/${item.evaluationId}`}
                    btnColor="btn-gray"
                  />
                )}
                {item.evaluationTypeName === "Six months probation review" && (
                  <TaskButton
                    path={`/evaluationSix/first/review/details/${item.evaluationId}`}
                    btnColor="btn-gray"
                  />
                )}
              </div>
