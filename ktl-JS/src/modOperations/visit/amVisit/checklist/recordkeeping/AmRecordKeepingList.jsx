...
const AmRecordKeepingList = ({ id, isSubmit }) => {
  const {
    data: list,
    error,
    isLoading,
    isError,
  } = useGetData("hrDepartment", `/amRecordKeeping/list/${id}`);
  ...
  return (
    <>
      <TopHeader title="4. Today’s Record Keeping information: " />
            ...
            >
              ...
              {isSubmit === 0 && (
                <div className="flex justify-end space-x-2">
                  <EditButton
                    path={`/ops/am/record/visit/edit/${item.opsAmRecordKeepingId}`}
                  />
                </div>
              )}
            </div>
...
export default AmRecordKeepingList;
