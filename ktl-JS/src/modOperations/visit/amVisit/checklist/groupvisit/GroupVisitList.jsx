...
const GroupVisitList = ({ id, isSubmit }) => {
  const {
    ...
    isError,
    refetch,
  } = useGetData("hrDepartment", `/amGroupVisit/list/${id}`);
            ...
            >
              ...
              {isSubmit === 0 && (
                <div className="flex justify-end space-x-2">
                  <EditButton
                    path={`/ops/am/group/visit/edit/${item.opsAmGroupvisitId}`}
                  />
                  <DeleteButton
                    action={refetch}
                    path={`/amgroupvisit/delete/${item.opsAmGroupvisitId}`}
                  />
                </div>
              )}
            </div>
