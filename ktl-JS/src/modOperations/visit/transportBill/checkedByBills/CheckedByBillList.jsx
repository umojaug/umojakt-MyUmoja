...
const CheckedByBillList = () => {
  const {
    ...
    isError,
    // refetch,
  } = useGetData("travelingBill", `/myTravelingBill/checkedByList`);
  ...
  return (
    <div className="card w-full max-w-screen-xl gap-2">
      <TopHeader
        title="Checked By Application list"
        btn="Return"
          ...
          list.data.map((item, index) => (
            ...
            >
              ...
              <div className="flex justify-end space-x-2">
                {item.isSubmit === 1 ? (
                  <TaskButton
                    path={`/ops/transportBill/checkedBy/details/${item.travelId}`}
                    btnColor={"btn-umojayellow"}
                  />
                ) : (
                  <TaskButton
                    path={`/ops/transportBill/checkedby/preview/${item.travelId}`}
                    btnColor="btn-gray"
                  />
                )}
...
export default CheckedByBillList;
