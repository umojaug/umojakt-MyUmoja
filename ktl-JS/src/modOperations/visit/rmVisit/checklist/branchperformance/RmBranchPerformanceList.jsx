...
const RmBranchPerformanceList = ({ id, isSubmit }) => {
  const {
    data: list,
    error,
    isLoading,
    isError,
    refetch,
  } = useGetData("rmBranchPerformancelist", `/rmBranchPerformance/list/${id}`);
            ...
            >
              ...
              {isSubmit === 0 && (
                <div className="flex justify-end space-x-2">
                  <EditButton
                    path={`/ops/rm/performance/visit/edit/${item.opsRmBranchPerformanceId}`}
                  />
                  <DeleteButton
                    action={refetch}
                    path={`/rmBranchPerformance/delete/${item.opsRmBranchPerformanceId}`}
                  />
                </div>
              )}
            </div>
...
export default RmBranchPerformanceList;
